CREATE OR REPLACE PROCEDURE ETL_STG_PRESENTACIONES IS

  -- VARIABLES GENERALES
--DECLARE 
  V_NOMBRE_PROCESO   VARCHAR2(30):= 'ETL_STG_PRESENTACIONES';
  V_FEC_INICIO       DATE;
  V_FEC_FIN          DATE;
  ...
  V_FONO_SMS         VARCHAR2(10) := '70341079';
...
BEGIN
  ...
  -- CODIGO DEL PROCESO
    ...
    EXECUTE IMMEDIATE 'TRUNCATE TABLE STG_PRESENTACIONES';
	
    INSERT INTO STG_PRESENTACIONES(ID_PRESENTACION, NOM_PRESENTACION, TAMAÑO, ID_PRODUCTO)
    SELECT ID_PRESENTACION, NOM_PRESENTACION, "TAMAÑO", ID_PRODUCTO FROM distribuidor.PRESENTACIONES;
